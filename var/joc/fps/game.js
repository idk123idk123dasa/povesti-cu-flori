...
function init() {
    ...
    // Initialize remote player model placeholder
    createRemotePlayer();
...
function meleeAttack() {
    ...
    raycasterMelee.far = 12.0;

    const hitIntersects = raycasterMelee.intersectObject(remotePlayer);
    if (hitIntersects.length > 0) {
        console.log("MELEE HIT ON OPPONENT!");
        if (networkReady && conn && conn.open) {
            conn.send({ type: "hit", damage: 50 });
        }
    }

function startRound() {
    if (playerWins >= MAX_WINS || opponentWins >= MAX_WINS) {
    ...
    // Clear old stuff
    for (const b of bullets) scene.load(b); // reuse bullets? No, scene.remove better
    bullets.forEach(b => scene.remove(b));
    bullets.length = 0;
    for (const i of impacts) scene.remove(i);
    impacts.length = 0;

    // Reset Player Position based on role
    if (isHost) {
    ...
    scoreDisplay.textContent = `Match: ${playerWins} - ${opponentWins}`;
}
...
function updateBullets() {
    for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        // Check for hits on the remote player
        const hitObjects = [remotePlayer, ...objects].filter(o => o !== null);
        const hitIntersects = bulletRay.intersectObjects(hitObjects);

        if (hitIntersects.length > 0) {
            const closestHit = hitIntersects[0];

            if (closestHit.object === remotePlayer) {
                // REMOTE PLAYER HIT
                let dmg = 20;
                const relativeY = closestHit.point.y - remotePlayer.position.y;
                const weaponType = b.userData.weaponType;

                if (relativeY > 3.5) { // Headshot
                    dmg = (weaponType === 'ak47') ? 100 : 70;
                    console.log("HEADSHOT ON OPPONENT!");
                }

                // Notify opponent they were hit
                if (networkReady && conn && conn.open) {
...
function initMultiplayer() {
    ...
    peer.on('open', (id) => {
        ...
        // Try to connect to peer based on hash
        if (roomName !== 'lobby') {
            lobby.style.display = 'block';
            lobbyStatus.textContent = "Connecting to room: " + roomName;

            // Simple logic: first one is host, second one connects
            // We'll use the hash as the ID for the host
            // NOTE: In a real app we'd use a server, but for 1v1 URL hash works

            // Actually, let's use a simpler logic:
            // One player provides their ID to the other, or we use a fixed ID for the room
            // For "same DNS" requirement, we'll try to find an opponent
            findOpponent(roomName);
        }
    });
...
function findOpponent(roomName) {
    const lobbyStatus = document.getElementById('lobby-status');
    // Try to connect to the "room" ID (the hash itself)
    // This is a naive matchmaking: person A uses 'room123' as their ID, person B connects to it

    // We'll regenerate the peer with a fixed ID based on hash if we want to be host
    peer.destroy();
    peer = new Peer(roomName);

    peer.on('open', (id) => {
        console.log('Acting as Host in room: ' + id);
        lobbyStatus.textContent = "Waiting for opponent in room: " + roomName;
    });

    peer.on('error', (err) => {
        if (err.type === 'unavailable-id') {
            // ID taken, so someone is already hosting. We connect as client.
    ...
    peer.on('connection', (connection) => {
        ...
        setupConnection();
        isHost = true;
        console.log('Client connected to our room!');
    });
}
...
function setupConnection() {
    ...
    conn.on('open', () => {
        networkReady = true;
        lobbyStatus.textContent = "Connected! Match starting...";
        setTimeout(() => {
...
function handleServerData(data) {
    if (data.type === 'move') {
        ...
        createOpponentBullet(data.pos, data.dir);
    } else if (data.type === 'hit') {
        takeDamage(data.damage);
    } else if (data.type === 'weapon') {
        // sync remote weapon model (visual only)
    }
}

function createRemotePlayer() {
    const loader = new THREE.TextureLoader();
    const texture = loader.load('character.png');
    ...
    remotePlayer.position.set(0, 7.5, -50);
    scene.add(remotePlayer);
}

function sendUpdate() {
    if (!networkReady || !conn || !conn.open) return;
...
function createOpponentBullet(pos, dir) {
    const bulletGeo = new THREE.SphereGeometry(0.5, 8, 8);
    const bulletMat = new THREE.MeshBasicMaterial({ color: 0xff0000 });
    const bullet = new THREE.Mesh(bulletGeo, bulletMat);
    bullet.position.copy(pos);

    const velocity = new THREE.Vector3(dir.x, dir.y, dir.z).multiplyScalar(1.2);
    bullet.userData.velocity = velocity;
    scene.add(bullet);
    // Add to a remote bullets array for tracking
    bullets.push(bullet);
}
